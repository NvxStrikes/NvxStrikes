<div align="center">

<!-- ANIMATED DYNAMIC HEADER -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=1,12,24,30&height=220&section=header&text=NOVASTRIKES&fontSize=68&fontAlignY=38&animation=twinkling&desc=3D%20Technical%20Artist%20%7C%20Creative%20Technologist%20%7C%20Builder&descAlignY=58&descAlign=50&fontColor=ffffff" width="100%"/>

<!-- LIVE ANIMATED TYPING TERMINAL -->
<a href="https://novastrikes.com/">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=21&duration=2800&pause=1000&color=00F2FE&center=true&vCenter=true&multiline=false&width=650&height=50&lines=Technical+3D+Artist+%26+Python+Developer;Crafting+Blender+Add-ons+%26+Workflow+Accelerators;Building+OpenQR+%7C+ZadeChat+%7C+Creative+SaaS;Turning+Complex+Geometry+Into+Clean+Software" alt="Typing SVG" />
</a>

<p align="center">
  <a href="https://novastrikes.com/"><img src="https://img.shields.io/badge/PORTFOLIO-novastrikes.com-000000?style=for-the-badge&logo=google-chrome&logoColor=00F2FE"/></a>
  <a href="https://superhivemarket.com/creators/novastrikes"><img src="https://img.shields.io/badge/SUPERHIVE_MARKET-Store-FF5722?style=for-the-badge&logo=gumroad&logoColor=white"/></a>
  <a href="https://twitter.com/novastrikeshq"><img src="https://img.shields.io/badge/X-@novastrikeshq-1DA1F2?style=for-the-badge&logo=x&logoColor=white"/></a>
</p>

</div>

---

### ⚡ The Mission & Focus

```bash
> nvx --status
  - 🎨 3D Engineering : Building custom Python pipelines, UI panels, and procedural tools for Blender.
  - 🚀 SaaS & Web      : Shipping high-performance modern web apps (OpenQR, ZadeChat).
  - 🛠️ Open Source     : Distributing free & pro workflow add-ons to eliminate repetitive friction for 3D creators.
```

---

### 🛠️ Tech Arsenal & Creative Stack

<div align="center">

<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=blender,py,ts,react,nextjs,nodejs,tailwind,docker,git,figma,postman,vscode&theme=dark" />
</a>

<br/>

| Domain | Tools & Technologies |
| :--- | :--- |
| **3D & DCC Engineering** | `Blender (bpy API)` · `Python` · `Cycles / Eevee` · `Shader Nodes` · `Geometry Nodes` |
| **Frontend & Products** | `React` · `Next.js` · `TypeScript` · `Tailwind CSS` · `UI/UX Design` |
| **Backend & Scripting** | `Python (Automation & Scraping)` · `Node.js` · `REST APIs` · `Git / GitHub Actions` |

</div>

---

### 📦 Featured Blender Add-ons & Flagship Tools

<div align="center">

| Project | Description | Stack | Status |
| :--- | :--- | :---: | :---: |
| **[Smart Autosave](https://github.com/NvxStrikes/smart-autosave-lite)** | Auto-backup manager with timeline checkpoints for Blender | `Blender` `Python` | ![Active](https://img.shields.io/badge/v1.0-Active-brightgreen?style=flat-square) |
| **[Flowboy](https://github.com/NvxStrikes/flowboy_lite)** | Instant N-Panel shortcut reference overlay for Object Mode | `Blender` `Python` | ![Active](https://img.shields.io/badge/v1.0-Active-brightgreen?style=flat-square) |
| **[Palette Manager](https://github.com/NvxStrikes/palette-manager-free)** | Persistent named color palette manager for 3D look-dev | `Blender` `Python` | ![Active](https://img.shields.io/badge/v1.0-Active-brightgreen?style=flat-square) |
| **[Smart Render Guard](https://github.com/NvxStrikes/smart-render-guard)** | Crash prevention & missing frame validator for render batches | `Blender` `Python` | ![Active](https://img.shields.io/badge/v1.0-Active-brightgreen?style=flat-square) |
| **[OpenQR](https://github.com/NvxStrikes)** | Modern QR generation platform & dynamic styling engine | `TypeScript` `React` | ![Building](https://img.shields.io/badge/WIP-In_Dev-orange?style=flat-square) |

</div>

---

### 🔥 Activity & Git Telemetry

<div align="center">

<!-- ANIMATED STREAK STATS -->
<img src="https://github-readme-streak-stats.herokuapp.com/?user=NvxStrikes&theme=tokyonight&hide_border=true&background=0D1117&fire=00F2FE&ring=7928CA&currStreakNum=00F2FE" alt="NovaStrikes Streak" height="155"/>

<br/>

<!-- STATS CARDS -->
<img src="https://github-readme-stats.vercel.app/api?username=NvxStrikes&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00F2FE&icon_color=FF5722&text_color=C9D1D9" height="155" alt="GitHub Stats" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=NvxStrikes&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00F2FE&text_color=C9D1D9" height="155" alt="Top Languages" />

</div>

---

<!-- ANIMATED FOOTER WAVE -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=1,12,24,30&height=90&section=footer" width="100%"/>
